...
trait TypeResolverTrait
{
    ...
    public function resolveType(\Reflector $reflector, ?Context $context): array
    {
        $typeResolver = $this->getTypeResolver($context);

        return [
            'docblock' => $typeResolver->getDocblockTypeDetails($reflector),
            'reflections' => $typeResolver->getReflectionTypeDetails($reflector),
        ];
    }
}
